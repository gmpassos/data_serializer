...
void main() {
  group('extension', () {
    ...
    test('ListGenericExtension', () {
      ...
      expect(
        l1.asUnmodifiableListView(),
        allOf(equals([1, 2, 3, 4, 5, 6, 7, 8]), isA<UnmodifiableListView>()),
      );

      expect(
        l1.asUnmodifiableListView().asUnmodifiableListView(),
        allOf(equals([1, 2, 3, 4, 5, 6, 7, 8]), isA<UnmodifiableListView>()),
      );
      ...
      expect(
        l2.toUint8List().reverseChunks(1),
    ...
    test('ListIntDataExtension', () {
      ...
      expect(
        l1.toUint8List().convertToUint16List(),
        allOf(equals([0x0102, 0x0304, 0x0506, 0x0708]), isA<Uint16List>()),
      );

      expect(
        l1.toUint8List().convertToUint16List(Endian.little),
        allOf(equals([0x0201, 0x0403, 0x0605, 0x0807]), isA<Uint16List>()),
      );

      expect(
        l1.toUint8List().convertToUint32List(),
    ...
    test('Uint32ListDataExtension', () {
      ...
      expect(
        ns32b.convertToUint16List(),
        equals([0x0002, 0x0304, 0x0506, 0x0708]),
      );
      expect(
        ns32b.convertToUint16List(Endian.big),
        equals([0x0002, 0x0304, 0x0506, 0x0708]),
      );
      expect(
        ns32b.convertToUint16List(Endian.little),
        equals([0x0200, 0x0403, 0x0605, 0x0807]),
      );

      expect(
        ns32b.convertToUint8List(),
        equals([0x00, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]),
      );
      expect(
        ns32b.convertToUint8List(Endian.big),
        equals([0x00, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08]),
      );
      expect(
        ns32b.convertToUint8List(Endian.little),
    ...
    test('Uint8ListDataExtension', () {
      ...
      expect(
        (bytes & [16, 16, 16, 16, 16, 16, 16, 16].toUint8List()),
        equals([0, 0, 0, 0, 0, 0, 0, 0]),
      );
      expect(
        (bytes & [1, 1, 1, 1, 1, 1, 1, 1].toUint8List()),
      ...
      expect(
        (bytes ^ [15, 15, 15, 15, 15, 15, 15, 15].toUint8List()),
        equals([14, 13, 12, 11, 10, 9, 8, 7]),
      );

      expect(
        [10, 20].toUint8List().group([30, 40].toUint8List()),
        equals([10, 20, 30, 40]),
      );
